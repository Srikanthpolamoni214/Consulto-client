...
const showSlots = (index, date) => {
  ...
  const filteredSlots = appointmentsData
    .filter(appoint => appoint.date === modifiedDate)
    .map(appoint => 
      appoint.bookedSlot ? String(appoint.bookedSlot).toLowerCase().trim() : ""
    )
    .filter(slot => slot !== ""); // remove empty ones

  setBookedSlots(filteredSlots);
console.log("...............", typeof doctor.avaliableslots[0])



  const totalAvailable = doctor.avaliableslots[0].split(",")?.length || 0;
  console.log(".........//", totalAvailable)
  const totalBooked = doctor.avaliableslots[0].split(",")?.filter(slot =>
    filteredSlots.includes(String(slot).toLowerCase().trim())
  ).length;
console.log(".....", totalBooked)
  if (totalAvailable === totalBooked) {
    ...
    const isSlotBooked = (time) => {
    if (!time) return false; // avoid crashing on null/undefined
    const timeStr = String(time).toLowerCase().trim();
    return bookedSlots.includes(timeStr);
};

    const bookSlot = (index) => {
        if (isSlotBooked(doctor.avaliableslots[0].split(",")[index])) return;
        setSelectedTimeIndex(index);
        setSelectedSlot(doctor.avaliableslots[0].split(",")[index]);
    };
                ...
                <section className="details-bookingContainer">
                    ...
                    <section className="bookingContainer" id="timeSlots" style={{ display: "none" }}>
                        ...
                        {isLoading ? (
                            ...
                            <div className="time-grid">
                                {doctor.avaliableslots[0].split(",")?.map((time, index) => {
...
>
  ...
  {isBooked && <span className="booked-badge">Booked</span>}
                    ...
                    <div style={{"display":"flex","justifyContent":"center"}}>                    <button className="booknowBtn" onClick={openForm}>BOOK APPOINTMENT</button>
</div>
                </section>
            </section>
